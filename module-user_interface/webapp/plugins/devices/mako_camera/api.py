...
def configure():
    if request.method == "POST":
        ...
        return Response(response=json.dumps(failed_features, default=str), status=200, mimetype="application/json")

    if request.method == "GET":
        mako_camera.cam_running_event.clear()
        return Response(
            response=json.dumps(mako_camera.list_features(), default=str), status=200, mimetype="application/json"
        )
